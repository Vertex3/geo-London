...
import os, sys, traceback, time, xml.dom.minidom, arcpy, gzSupport, urllib, urllib.parse, urllib.request, webbrowser, myGizinta
...
def OpenBrowserURL(xmlFileName):
    ...
    params = urllib.parse.urlencode(theData)
    url = url + "?" + str(params)
    setupProxy()
